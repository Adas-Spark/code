...
For emailing participants of the 2025 5K I did the below
1. Download tickets from Givebutter (there is an export option under "Event", "Attendees")
        a. Export just the first name, last name, email, and ticket type column.
2. Exclude virtual tickets:
...
grep "Virtual" tickets-2025-06-02-467515950.csv|cut -d',' -f1,2,3>tickets-2025-06-02-467515950_VIRTUAL_cleaned.csv# Remember to delete the header line if it exists...just do it manually with vi
3. Save email template as a docx (using Google Docs)
4. Open Jupyter Lab in Ada's Spark profile:
conda activate gmail_gcal_interface
jupyter lab --no-browser
5. run mass_email_app_v2.ipynb
